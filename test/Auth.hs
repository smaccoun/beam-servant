...
loginTestUser :: (MonadReader r m, MonadIO m, HasDBConn r) => JWK -> m (Either WaiTestFailure Text)
loginTestUser jwk = do
  eitherLoginResponse <- loginUserPassword (defaultJWTSettings jwk) validTestUserLogin
